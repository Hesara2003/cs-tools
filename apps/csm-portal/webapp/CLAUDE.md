...
**That same `minWidth: "max-content"` fix has its own follow-on gotcha: it makes a *default* (always-visible) free-text column able to force the whole grid to scroll on its own**, with zero optional columns turned on — reported live once a case with an unusually long subject line did exactly that. `noWrap`'s ellipsis (used throughout this file — Subject, product, customer, assignee, createdBy) only clips *paint*; it has no effect on a grid item's intrinsic max-content sizing, which is what `minWidth: "max-content"` on the container actually keys off. The fix is a real `maxWidth` (a fixed px value, not a percentage) on the cell's own wrapping `Box` — a grid item's specified max-width *does* clamp its max-content contribution to track sizing, unlike `overflow`/`text-overflow`. Only Subject has one today (`maxWidth: 480` — it's the one default column holding genuinely unbounded free text; Case ID/State are short and self-bounding). Any other default column that starts holding long freeform text needs the same treatment — an *optional* column growing the grid when added is fine and intended (that's what the fix above is for); it's only a *default*, always-on column doing it unprompted that's the bug.
